<template>
  <div>
    ...
    <div class="page-container text-center">
      ...
      <div>
        <div class="title">{{ Content.info_message.verify[$i18n.locale] }}</div>
        <div class="my-1">
          <div class="input-title">{{ Content.info_message.number[$i18n.locale] }}</div>
          <input-element
            class="input"
            type="number"
            ...
            v-model="mobileNo"
          />
          <div v-show="!isValidMobileNo" class="err-msg text-left">{{ Content.info_message.mobileNoErrMsg[$i18n.locale] }}</div>
        </div>
        <button-element
        ...
        <div v-else-if="!otp.otpLoading && otp.otpReceived" class="my-2">
          <div class="input-title">{{ Content.otp.fill_otp[$i18n.locale] }}</div>
          <div class="otp-input">
            <input-element
              class="input"
              type="number"
          ...
          <div v-show="error" class="err-msg text-center mt-1">
            {{ errorMessage }}
          </div>
          <button-element
            class="my-1"
...
<script>
  ...
  export default {
    ...
    mounted () {
      ...
      const campCode = { campCode: this.campId }
        ...
        this.$store.dispatch(VERIFY_OTP_MOBILE_ONLY, body)
          .then((data) => {
            ...
            if (!data.is_pass) {
              ...
              if (this.$route.name === 'clmbundle') localStorage.setItem('is-clmbundle-campain', 'clmbundle')
              this.$router.push(`/online-store?product_id=${this.productId}`)
            }
          })
          .catch((error) => {
            this.isVerifying = false

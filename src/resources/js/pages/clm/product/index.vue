...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      productId () {
        const queries = Object.keys(this.$route.query);
        const parameter = getProductQueries(queries)
        if (!parameter) return ''
        return this.$route.query[parameter];
      },
      completedInput () {
      ...
      bannerUrl () {
        if (this.instantBenefit && this.instantBenefit.banner) {
          ...
          return `/images/store/clm-banner-${this.clmType}.jpg`
        }
      }
    },
    watch: {
        ...
        this.$store.dispatch(VERIFY_OTP_MOBILE_ONLY, body)
          .then((data) => {
            ...
            if (!data.is_pass) {
              ...
              if (body.type === 'sim') {
                this.$router.push(`/online-store/postpaid/${this.productId}?nas_code=${data.nas_code_redirect}`)
              } else {
                this.$router.push(`/online-store/item/${this.productId}`)
              }
            }
          })
          .catch((error) => {

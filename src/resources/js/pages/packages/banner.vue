<template>
  <div class="banner-container">
    ...
    <div v-else class="skeleton dark animate" />
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    computed: {
      banners () {
        return (
          this.$store.state.postpaid.postpaidBanner.data &&
          this.$store.state.postpaid.postpaidBanner.data.record.sections[0].contents)
      },
      isMobile () {
    ...
    methods: {
      image (item) {
        return this.isMobile ? `${item.custom_fields.base_url}${item.custom_fields.banner_mobile}` : `${item.custom_fields.base_url}${item.banner}`
      },
      trackPromoView () {

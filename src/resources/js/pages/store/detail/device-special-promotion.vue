<template>
    <div
        v-if="specialPromotions && specialPromotions.length > 0"
        class="grid scroll-container"
    >
        <button
        aria-label="Scroll left"
        class="hidden lg:grid place-items-center p-1"
        @click="scroll('specialPromotions', -300)"
        >
        ...
        ref="specialPromotions"
        >
        <button
            v-for="(item, key) in specialPromotions"
            :key="key"
            :data-test-special-promotion-id="item.id"
            class="rounded-xl w-full"
            :class="{'shadow-lg': promotion === item.id && isSpecialPromotion}"
            style="width: 150px; padding: 3px;"
            @click="changePromotions(item.id, true, item)"
        >
            <div
            class="flex flex-col p-2 rounded-lg text-left shadow-gray-500 bg-black-gray-gradient text-white h-full hover:shadow-thick-red-500"
            ...
            <div class="text-20 font-light" v-if="typeof productInfo.config_free_device !== 'undefined' && typeof productInfo.config_free_device[item.id] !== 'undefined' && item.haveFreeDevice" v-html="productInfo.config_free_device[item.id].label_black_card[$i18n.locale]"></div>
            <div class="text-20 font-light" v-else>{{ Locale.start_at[$i18n.locale] }}</div>
            <div class="flex justify-between items-end">
                <div class="text-3xl font-bold">{{ item.min_price.toLocaleString() }}.-</div>
                <div
                class="grid place-items-center border border-white rounded-full w-[20px] h-[20px]"
                ...
                <svg width="14" height="10" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path
                      ...
                      :fill="`${promotion === item.id && isSpecialPromotion? '#FFF' : '#FFF'}`"
                    />
                </svg>
                </div>
            </div>

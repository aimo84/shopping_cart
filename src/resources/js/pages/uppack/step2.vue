...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      userData () {
        return this.$store.state.clm.clmPackageList.clm_verify_redis_data_json_decode?.input
      },
      packages () {
        return this.$store.state.clm.clmPackageList.tierBeanList ? this.$store.state.clm.clmPackageList.tierBeanList.map((tier) => {
          const offerList = this.$store.state.clm.clmPackageList.moiTierList ? this.$store.state.clm.clmPackageList.moiTierList.offerSummaryList.filter((offer) => {
    ...
    methods: {
      ...
      async fetchClmPackages () {
        ...
        this.$store.dispatch(GET_LIST_PACKAGE, data)
          .then((data) => {
            this.isLoading = false
            if (data.step_3_clm && data.step_3_clm.campaignInfoBeanList.length > 0 && data.step_3_clm.campaignInfoBeanList[0].campaignBean
              && data.step_3_clm.campaignInfoBeanList[0].campaignBean.knowledgeDetailList && data.step_3_clm.campaignInfoBeanList[0].campaignBean.knowledgeDetailList.length > 0)
            {
              const headlineObj = data.step_3_clm.campaignInfoBeanList[0].campaignBean.knowledgeDetailList.filter(k => k.topic === 'Headline')
              let headline = '-'
              let img = ''
              ...
              const subHeadline = data.step_3_clm.campaignInfoBeanList[0].campaignBean.knowledgeDetailList.filter(k => k.topic === 'Sub Headline').length > 0 ? data.step_3_clm.campaignInfoBeanList[0].campaignBean.knowledgeDetailList.filter(k => k.topic === 'Sub Headline')[0].script : '-'
              const terms = data.step_3_clm.campaignInfoBeanList[0].campaignBean.knowledgeDetailList.filter(k => k.topic === 'Term and Conditions').length > 0 ? data.step_3_clm.campaignInfoBeanList[0].campaignBean.knowledgeDetailList.filter(k => k.topic === 'Term and Conditions')[0].script : '-'
